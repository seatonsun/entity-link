...
def getEntity(mention,entityname):
    ...
    if mention == '':
        ...
        conn= MySQLdb.connect(host='localhost',user='root',passwd = '123',db ='entitylinkdb')
        cur = conn.cursor()
...
def getPairPath(entitypair):
    ...
    "mysql6:a<-()->b"
    patharray = []
    conn= MySQLdb.connect(host='localhost',user='root',passwd = '123',db ='entitylinkdb',port=3306)
    cur = conn.cursor()
